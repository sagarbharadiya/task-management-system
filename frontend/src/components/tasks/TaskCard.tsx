...
import { Edit, Trash2, User, Check, X } from 'lucide-react';
...
interface TaskCardProps {
  ...
  onUpdate?: (taskId: string, updates: { title?: string; description?: string; priority?: TaskPriority }) => Promise<void>;
}
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, onDelete, onUpdate }) => {
  ...
  const handleDescriptionSave = async () => {
    if (editedDescription.trim() && editedDescription !== task.description && onUpdate) {
      try {
        await onUpdate(task.id, { 
          title: task.title,
  ...
  // Handle key presses
  const handleKeyPress = (e: React.KeyboardEvent, type: 'title' | 'description' | 'priority') => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          {isEditingPriority ? (
            ...
            <span
              className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border cursor-pointer hover:opacity-80 transition-opacity ${getPriorityColor(task.priority)}`}
